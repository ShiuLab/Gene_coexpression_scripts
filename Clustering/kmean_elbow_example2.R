...
data1 <- read.table(file='/mnt/home/uygunsah/1_Expression_Database/expression_matrix/5_Stress_FC', sep='\t', header=T, row.names=1)
...
for (i in seq(5, n.lev, 50)) wss[i] <- sum(kmeans(kdata, centers=i, iter.max=100, nstart=50)$withinss)} #for (i in seq(0, 10, 2)) print(i)

# Calculate the within groups SSE for 250 randomized data sets (based on the original input data)
k.rand <- function(x){
km.rand <- matrix(sample(x),dim(x)[1],dim(x)[2])
rand.wss <- as.matrix(dim(x)[1]-1)*sum(apply(km.rand,2,var))
for (i in seq(5, n.lev, 50)) rand.wss[i] <- sum(kmeans(km.rand, centers=i, iter.max=100, nstart=50)$withinss)
...
km.rand <- matrix(sample(x),dim(x)[1],dim(x)[2])
rand.wss <- as.matrix(dim(x)[1]-1)*sum(apply(km.rand,2,var))
for (i in seq(5, n.lev, 50)) rand.wss[i] <- sum(kmeans(km.rand, centers=i, iter.max=50, nstart=10)$withinss)
rand.wss <- as.matrix(rand.wss)
return(rand.wss)}
k.2 <- function(x){
for (i in 1:250) {
...
#fit <- kmeans(kdata, clust.level, iter.max=100, nstart=50)
